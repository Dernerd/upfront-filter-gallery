    ...
    upfront_image_gallery = {
        ...
        init: function() {
            ...
            jQuery('#add-new-images').on('click', function(event) {
                ...
                file_frame.on('select', function() {
                    ...
                    for (var i = 0; i < length; i++) {
                        upfront_image_gallery.get_thumbnail(images[i]['id']);
                    }
                });
                file_frame.open();
            });
			...
			jQuery('#remove-all-images').on('click', function() {
                if (confirm('Are sure to delete all images?')) {
                    upfront_image_gallery.ul.empty();
                }
                return false;
            });
        ...
        get_thumbnail: function(id, cb) {
            ...
            jQuery.post(ajaxurl, data, function(response) {
                upfront_image_gallery.ul.append(response);
                cb();
            });
        }
    };
    upfront_image_gallery.init();
});