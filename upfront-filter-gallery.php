...
/*
Plugin Name:	UpFront Filtergalerie
Plugin URI:		https://n3rds.work
Description:  	Filtergalerie für Wordpress. Basierend auf Filter Gallery 0.0.1 von A WP Life
Version:		1.0.3
Author: 		WMS N@W
Author URI: 	https://n3rds.work
License:      	GPL2
License URI:  	https://www.gnu.org/licenses/gpl-2.0.html
Text Domain:  	upfront-filter-gallery
Domain Path:  	/languages


UpFront Filter Gallery plugin is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
any later version.
 
UpFront Filter Gallery plugin is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.
 
You should have received a copy of the GNU General Public License
along with UpFront Filter Gallery plugin. If not, see https://www.gnu.org/licenses/gpl-2.0.html.
*/
require 'includes/psource-plugin-update/plugin-update-checker.php';
$MyUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
	'https://n3rds.work//wp-update-server/?action=get_metadata&slug=upfront-filter-gallery', 
	__FILE__, 
...
if ( ! class_exists( 'UpFront_Filter_Gallery' ) ) {

	class UpFront_Filter_Gallery {		
		...
		protected function _constants() {
			...
			//Plugin Text Domain
			define("PFG_TXTDM","upfront-filter-gallery" );

			//Plugin Name
			define( 'PFG_PLUGIN_NAME', __( 'UpFront-Filtergalerie', PFG_TXTDM ) );

			//Plugin Slug
			define( 'PFG_PLUGIN_SLUG', 'upfront_filter_gallery' );
		...
		protected function _hooks() {
			...
			//Create UpFront Filter Gallery Custom Post
			add_action( 'init', array( $this, 'UpFront_Filter_Gallery' ));
		...
		public function pfg_menu() {
			$filter_menu = add_submenu_page( 'edit.php?post_type='.PFG_PLUGIN_SLUG, __( 'Filters', PFG_TXTDM ), __( 'Filters', PFG_TXTDM ), 'administrator', 'pfg-filter-page', array( $this, 'upfront_filter_page') );
		...
		public function UpFront_Filter_Gallery() {
			$labels = array(
				'name'                => _x( 'UpFront Filter Gallery', 'Post Type General Name', PFG_TXTDM ),
				'singular_name'       => _x( 'UpFront Filter Gallery', 'Post Type Singular Name', PFG_TXTDM ),
				...
				'new_item'            => __( 'New UpFront Filter Gallery', PFG_TXTDM ),
				'edit_item'           => __( 'Edit UpFront Filter Gallery', PFG_TXTDM ),
				'update_item'         => __( 'Update UpFront Filter Gallery', PFG_TXTDM ),
				'search_items'        => __( 'Search UpFront Filter Gallery', PFG_TXTDM ),
				'not_found'           => __( 'UpFront Filter Gallery Not found', PFG_TXTDM ),
				'not_found_in_trash'  => __( 'UpFront Filter Gallery Not found in Trash', PFG_TXTDM ),
			);
			$args = array(
				'label'               => __( 'UpFront Filter Gallery', PFG_TXTDM ),
				'description'         => __( 'Custom Post Type For UpFront Filter Gallery', PFG_TXTDM ),
				'labels'              => $labels,
			...
			register_post_type( 'upfront_filter_gallery', $args );
		} // end of post type function
		
		public function admin_add_meta_box() {
			add_meta_box( __('Add UpFront Filter Gallery', PFG_TXTDM), __('Add UpFront Filter Gallery', PFG_TXTDM), array(&$this, 'pfg_image_upload'), 'upfront_filter_gallery', 'normal', 'default' );
		}
		...
		public function pfg_image_upload($post) {		
			...
			<div id="image-gallery">
				...
				<ul id="remove-images" class="sbox">
					<?php
					$allimagesetting = unserialize(base64_decode(get_post_meta( $post->ID, 'upfront_filter_gallery'.$post->ID, true)));
					
					$all_category = get_option('upfront_portfolio_filter_gallery_categories');
			...
			if(isset($_POST['pfg_save_nonce'])) {
				if (!isset( $_POST['pfg_save_nonce'] ) || ! wp_verify_nonce( $_POST['pfg_save_nonce'], 'pfg_save_settings' ) ) {
				   ...
				   exit;
					...
					$upfront_image_gallery_shortcode_setting = "upfront_filter_gallery".$post_id;
					update_post_meta($post_id, $upfront_image_gallery_shortcode_setting, base64_encode(serialize($_POST)));
				}
			}
		}// end save setting
		
		//filter/category page
		public function upfront_filter_page() {
	...
	$pfg_portfolio_gallery_object = new UpFront_Filter_Gallery();		
